...
- Uses the same `view {}` syntax (no separate native-only view block).
...
- Styling output is still CSS-oriented; the native target does not consume `app.css` yet.

## Dependencies (Linux)

- The native build uses `clang++` and (when Sokol is present) links against `X11`, `Xi`, `Xcursor`, and `GL`.
...
- **Native backend**: retained tree + layout + renderer + input/event loop
