...
static std::unordered_map<std::string, std::unique_ptr<CachedImage>> g_images;
...
const NativeImageInfo* native_image_get_or_load(const char* src) {
    ...
    if (it != g_images.end()) return it->second ? &it->second->info : nullptr;
    ...
    if (!rgba || w <= 0 || h <= 0) {
        ...
        g_images.emplace(key, nullptr);
        return nullptr;
    }

    auto img = std::make_unique<CachedImage>();
    img->w = w;
    img->h = h;
    ...
    img->info.w = img->w;
    img->info.h = img->h;
    img->info.image_data = &img->scl;

    const NativeImageInfo* out = &img->info;
    g_images.emplace(key, std::move(img));
    return out;
}
...
const NativeImageInfo* native_image_get_or_load(const char*) { return nullptr; }
void native_image_cache_shutdown() {}
