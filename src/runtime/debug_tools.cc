// Debug tools implementation (compiled as a TU).
    ...
    for (int32_t i = 0; i < render_commands.length; i++) {
        ...
        if (cmd->commandType == CLAY_RENDER_COMMAND_TYPE_SCISSOR_START) {
            ...
            if (itn != coi::ui::g_nodes.end()) {
	            ...
	            // Some layout engines emit "between-children" borders as anonymous RECTANGLE commands. Expose those in the dump.
            const int32_t owner = cmd->userData ? (int32_t)(intptr_t)cmd->userData : 0;
            if (owner != 0) {
...
void flush() {
    ...
    // Native scripts and simulated input should run during the actual main loop.
