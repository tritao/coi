...
class RmlUiBackend final : public UiBackend {
  public:
    ...
    void init_gfx() override {
        ...
        if (font_path_env && *font_path_env) {
            font_path = resolve_existing_path(std::filesystem::path(font_path_env));
        }
        if (font_path.empty()) {
    ...
    void layout(float w, float h, float dpi) override {
        ...
        if (pending_input.scroll_x != 0.0f || pending_input.scroll_y != 0.0f) {
            // Convert internal scroll convention back to browser-like wheel deltas: positive is right/down.
