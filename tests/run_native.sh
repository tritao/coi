...
TEST_ROOTS=("$PROJECT_ROOT/tests/native/runtime" "$PROJECT_ROOT/tests/native/compile")
...
if [ "$missing" -ne 0 ]; then
  echo -e "${RED}No native tests found (expected runtime/ and compile/ under tests/native)${NC}"
  exit 1
fi
...
if [ "$TOTAL" -eq 0 ]; then
  echo -e "${RED}No native tests found in tests/native/runtime or tests/native/compile${NC}"
  exit 1
fi
...
while IFS= read -r -d '' test_file; do
  ...
  set +e
  compiler_out="$("$COMPILER" "$test_file" --target native --out "$dist_dir" 2>&1)"
  compiler_rc=$?
  set -e
  ...
  if [ ! -f "$dist_dir/app" ]; then
    FAILED_TESTS+=("$rel_path (native build did not produce dist/app)")
    FAILURES=$((FAILURES+1))
    rm -rf "$tmp_dir"
    draw_progress_bar $((PASSED + FAILURES)) "$TOTAL"
    continue
  ...
  run_env=(COI_NATIVE_DUMP="$dump_mode" COI_NATIVE_FRAMES="$frames")
  if [ "$layout_dump_set" -eq 1 ]; then
    run_env+=(COI_NATIVE_LAYOUT_DUMP="$layout_dump")
  fi
  if [ "$render_dump_set" -eq 1 ]; then
    run_env+=(COI_NATIVE_RENDER_DUMP="$render_dump")
  fi
  if [ -n "$viewport" ]; then
    run_env+=(COI_NATIVE_VIEWPORT="$viewport")
  fi
  if [ -n "$click" ]; then
    run_env+=(COI_NATIVE_CLICK="$click")
  fi
  if [ -n "$pointer" ]; then
    run_env+=(COI_NATIVE_POINTER="$pointer")
  fi
  if [ -n "$scroll" ]; then
    run_env+=(COI_NATIVE_SCROLL="$scroll")
  fi
  if [ -n "$script_path" ]; then
    run_env+=(COI_NATIVE_SCRIPT="$script_path")
  fi
  if [ -n "$script_dumps" ]; then
    run_env+=(COI_NATIVE_SCRIPT_DUMPS="$script_dumps")
  fi
...
if [ $FAILURES -eq 0 ]; then
  echo -e "${GREEN}All $TOTAL native test(s) passed!${NC}"
  exit 0
fi

echo -e "${RED}$FAILURES native test(s) failed:${NC}"
